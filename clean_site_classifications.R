...
readr::read_csv(here::here('derived_data/media_classification/adfontes_raw.csv')) %>%
...
readr::read_csv(here::here('derived_data/media_classification/allsides_raw.csv')) %>%
  mutate(leaning_allsides = case_when(`0` %in% c("left-center", "left") ~ 'L',
                                  `0` %in% c("right-center", "right") ~ 'R',
                                  `0` %in% c("center") ~ 'C')) %>%
  filter(!is.na(leaning_allsides)) %>%
  select(channel = index, leaning_allsides) %>%
  mutate(channel = tolower(stringr::str_replace_all(channel, '-', ' '))) -> allsides

readr::read_csv(here::here('derived_data/media_classification/mediabiasfactcheck_raw.csv')) %>%
  select(channel = name, leaning_mbfc = leaning) %>%
  mutate(leaning_mbfc = forcats::fct_collapse(as.factor(leaning_mbfc),
                                         L = c("left extreme", "left", "left center"),
                                         R = c("right extreme", "right", "right center"),
                                         C = "center",
                                         group_other = TRUE)) %>%
...
readr::read_csv(here::here('derived_data/video_info.csv')) %>%
...
channels_matched %>%
  write_csv(here::here('derived_data/channel_classification.csv'))
